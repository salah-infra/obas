      ...
      <div className="mx-auto max-w-[80rem]">
        ...
        <div role="tablist" className="mt-8 flex flex-wrap gap-x-6 gap-y-2 border-b" style={{ borderColor: 'var(--line)' }}>
          {t.solutions.tabs.map((tb, i) => (
            <button key={tb.id} role="tab" id={`tab-${tb.id}`} aria-controls="solutions-panel" aria-selected={i === active} onClick={() => setActive(i)}
        ...
        <AnimatePresence mode="wait">
          <motion.div key={tab.id} id="solutions-panel" role="tabpanel" aria-labelledby={`tab-${tab.id}`} tabIndex={0} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.25 }} className="pt-8">
